...
static void init_buffers(guint position_index, guint color_index, guint* vao_out) {
    ...
    glVertexAttribPointer(position_index, 3, GL_FLOAT, GL_FALSE, sizeof(struct vertex_info),
                          (GLvoid*)(G_STRUCT_OFFSET(struct vertex_info, position)));

    /* enable and set the color attribute */
    glEnableVertexAttribArray(color_index);
    glVertexAttribPointer(color_index, 3, GL_FLOAT, GL_FALSE, sizeof(struct vertex_info),
...
static void adjustment_changed(GlareaAppWindow* self, GtkAdjustment* adj) {
    ...
    compute_mvp(self->mvp, self->rotation_angles[X_AXIS], self->rotation_angles[Y_AXIS],
...
static gboolean animate_rotation(GtkWidget* widget, GdkFrameClock* frame_clock, gpointer data) {
    ...
    gtk_adjustment_set_value(self->x_adjustment,
                             fmod(gtk_adjustment_get_value(self->x_adjustment) + angle, 360.0));
    gtk_adjustment_set_value(self->y_adjustment,
                             fmod(gtk_adjustment_get_value(self->y_adjustment) + angle, 360.0));
    gtk_adjustment_set_value(self->z_adjustment,
                             fmod(gtk_adjustment_get_value(self->z_adjustment) + angle, 360.0));

    return G_SOURCE_CONTINUE;
}

static void animate_toggled(GlareaAppWindow* self) {
...
static void glarea_app_window_class_init(GlareaAppWindowClass* klass) {
    ...
    gtk_widget_class_set_template_from_resource(widget_class,
                                                "/io/bassi/glarea/glarea-app-window.ui");
