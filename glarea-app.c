...
static void glarea_app_startup(GApplication* app) {
    GtkBuilder* builder;
    GMenuModel* app_menu;
...
static void glarea_app_activate(GApplication* app) {
    GlareaApp* self = GLAREA_APP(app);

    if (self->window == NULL) self->window = glarea_app_window_new(GLAREA_APP(app));

    gtk_window_present(GTK_WINDOW(self->window));
}

static void glarea_app_class_init(GlareaAppClass* klass) {
...
static void glarea_app_init(GlareaApp* self) {}
